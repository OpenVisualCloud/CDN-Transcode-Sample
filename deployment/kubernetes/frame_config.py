...
class MyFrame_Config (wx.Frame):
            ...
            if re.search("live\d", podname):
                for num in range(4):
                    for option in ["codec", "protocol", "resolution", "bitrate"]:
                        getattr(self, "%s_transcode%d_%s_choice" %
                                (podname, num, option)).SetSelection(-1)
            wx.CallAfter(self.start_thread, self.loginfo)
        return fun
    ...
    def creat_module_panel(self, arguments_label, modulename):  # live/vod
    ...
    def menu_sdbSizerOnOKButtonClick(self, event):
        ...
        update_yaml.update_yaml(nfs_server=self.nfs_server, volume_directory=self.volume_directory,
