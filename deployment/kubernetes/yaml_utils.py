...
def update_command(data, imageName, live_args):
    ...
    if imageName == "hw":
        command = 'ffmpeg -re -stream_loop -1 -hwaccel vaapi -hwaccel_device /dev/dri/renderD128 -hwaccel_output_format vaapi -i /var/www/archive/' + \
            live_args['input_video']
    else:
        command = 'ffmpeg -re -stream_loop -1 -i /var/www/archive/' + \
            live_args['input_video']

    for key, value in live_args['output_dict'].items():
        command += ' -vf ' + scale_dict[imageName] + '=' + value[1] + ' -c:v ' + value[3] + ' -b:v ' + \
            value[2] + 'M -g 32 -forced-idr 1 -an -f flv rtmp://cdn-service/' + \
            value[0] + '/' + key
