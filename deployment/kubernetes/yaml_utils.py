...
def update_command(data, imageName, num, live_args_list):
    ...
    av1_cmd = ' -forced-idr 1' if live_args_list[num]['codec_type'] == "libsvt_av1" else ''
    if imageName == "hw":
    ...
    for key, value in live_args_list[num]['output_dict'].items():
        command += ' -vf ' + scale_dict[imageName] + '=' + value[1] + ' -c:v ' + live_args_list[num]['codec_type'] + ' -b:v ' + value[2] + 'M -g 32' + av1_cmd + ' -an -f flv rtmp://cdn-service/' + value[0] + '/' + key + "_" + str(num)
